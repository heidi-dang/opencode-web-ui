...
import { backendToServerConnection, fetchBootstrap, bootstrapToServerConnections, findBootstrapBackend, normalizedBackendUrl, type BootstrapResponse } from "@/utils/control-plane"
...
export const { use: useServer, provider: ServerProvider } = createSimpleContext({
  ...
  init: (props: {
    ...
    const allServers = createMemo((): Array<ServerConnection.Any> => {
      ...
      const stored = store.list
        .filter((value) => {
          ...
          return !authoritative || !candidate.id || authoritative.some((server) => server.http.id === candidate.id)
        })
        .map((value) => {
        ...
        for (const item of legacy) {
          ...
          try {
            const response = await fetch("/api/opencode/servers", {
              ...
              body: JSON.stringify({ baseUrl: http.url, ...credentials.get(item.index) }),
            })
            ...
            // Preserve the safe URL-only legacy entry for a later retry.
          }
        }
    ...
    createEffect(() => {
      ...
      if (preferred) {
        const active = list.find((conn) => conn.type === "http" && conn.http.id === preferred)
        if (active && state.active !== preferred) {
          setState("active", preferred as ServerConnection.Key)
          return
        }
      }
