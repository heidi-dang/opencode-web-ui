...
export function assertNetworkPolicy(endpoint: string, policy: NetworkPolicy = {}) { const url = new URL(endpoint); if (!policy.allowPrivate) { const hostname = url.hostname.toLowerCase(); const isPrivate = hostname === "localhost" || hostname === "[::1]" || hostname === "::1" || /^127\./.test(hostname) || /^10\./.test(hostname) || /^192\.168\./.test(hostname) || /^172\.(1[6-9]|2\d|3[01])\./.test(hostname) || /^169\.254\./.test(hostname) || /^100\.(6[4-9]|[7-9]\d|1\d\d)\./.test(hostname) || /^fc[0-9a-f]{2}:/i.test(hostname) || /^fe[89ab][0-9a-f]:/i.test(hostname); if (isPrivate && !isExplicitlyAllowedPrivateOrigin(url)) throw new Error("PRIVATE_NETWORK_NOT_ALLOWED") } return url }
