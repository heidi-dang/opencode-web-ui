...
export class AgentBackendManager {
  ...
  async health(id: string, recovery = false, signal?: AbortSignal) { const backend = await this.get(id); const circuit = this.circuits.get(id)!; if (!recovery && !circuit.canRequest()) throw new Error("BACKEND_CIRCUIT_OPEN"); if (recovery && !circuit.tryRecoveryProbe() && !circuit.canRequest()) throw new Error("BACKEND_CIRCUIT_OPEN"); try { const result = await backend.health(signal); if (result.healthy) circuit.success(); else circuit.failure(result.authenticated); return result } catch (error) { circuit.failure(error instanceof Error && !/^BACKEND_HTTP_(401|403)$/.test(error.message)); throw error } }
