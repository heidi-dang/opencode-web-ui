  ...
  return (
    <section class="flex min-h-0 flex-col rounded-2xl border border-border-weak-base bg-surface-raised-strong shadow-[0_16px_40px_rgba(0,0,0,0.18)]" aria-labelledby="session-lineage-title">
      <header class="flex items-center justify-between gap-3 border-b border-border-weak-base px-4 py-3">
        <div class="min-w-0">
          <p id="session-lineage-title" class="text-14-emphasis text-text-strong">{language.t("autonomousWorkspace.lineage.title")}</p>
          <p class="text-12-regular text-text-weak">{language.t("autonomousWorkspace.lineage.description")}</p>
        </div>
        <span class="rounded-full bg-surface-base-hover px-2 py-1 text-11-regular text-text-muted">
          {language.t("autonomousWorkspace.lineage.count", { count: props.sessions().length })}
        </span>
      </header>
