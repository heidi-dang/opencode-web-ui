...
export function AutonomousWorkspace(props: {
  ...
  return <main class="flex min-h-0 flex-1 flex-col bg-background">
    <header class="workspace-header flex shrink-0 flex-col gap-2 border-b border-border-weak-base bg-surface-raised-strong px-3 py-2.5 sm:px-5">
      <div class="flex min-w-0 items-center justify-between gap-3">
        <div class="flex min-w-0 items-center gap-2.5">
          <div class="grid size-7 shrink-0 place-items-center rounded-md bg-surface-info-weak text-12-emphasis text-text-info" aria-hidden="true">◎</div>
          <div class="min-w-0"><p class="truncate text-13-emphasis text-text-strong">{language.t("autonomousWorkspace.title")}</p><p class="hidden text-11-regular text-text-weak sm:block">{language.t("autonomousWorkspace.description")}</p></div>
        </div>
        <span class="shrink-0 font-mono text-11-regular text-text-muted" aria-live="polite">{language.t("autonomousWorkspace.timeline.count", { count: props.events().length })}</span>
      </div>
      <nav class="workspace-tabs flex min-w-0 gap-0.5 overflow-x-auto pb-0.5" role="tablist" aria-label={language.t("autonomousWorkspace.views.label")}>
        <For each={views()}>{(item) => <button type="button" role="tab" aria-selected={view() === item.id} tabIndex={view() === item.id ? 0 : -1} class={`workspace-tab shrink-0 border-b-2 px-2.5 py-1.5 text-11-emphasis transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus ${view() === item.id ? "border-border-focus text-text-strong" : "border-transparent text-text-muted hover:text-text-base"}`} onClick={() => setView(item.id)} onKeyDown={(event) => { if (event.key === "ArrowRight" || event.key === "ArrowDown") { event.preventDefault(); const index = views().findIndex((entry) => entry.id === item.id); setView(views()[(index + 1) % views().length].id) } if (event.key === "ArrowLeft" || event.key === "ArrowUp") { event.preventDefault(); const index = views().findIndex((entry) => entry.id === item.id); setView(views()[(index - 1 + views().length) % views().length].id) } }}>{item.label}</button>}</For>
      </nav>
    </header>
...
export function WorkspaceModeToggle(props: { enabled: () => boolean; onToggle: () => void }) {
  const language = useLanguage()
  return <button type="button" aria-pressed={props.enabled()} class="workspace-mode-toggle inline-flex min-h-10 items-center rounded-md border px-2.5 text-11-emphasis transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus" classList={{ "border-border-focus bg-surface-info-weak text-text-info": props.enabled(), "border-border-weak-base bg-surface-raised-strong text-text-muted hover:bg-surface-base-hover": !props.enabled() }} onClick={props.onToggle}>{props.enabled() ? language.t("autonomousWorkspace.toggle.disable") : language.t("autonomousWorkspace.toggle.enable")}</button>
}
