...
export const HarnessFindingSchema = z.object({
  ...
  recommendedVehicle: z.string().min(1),
...
export const HarnessReportSchema = z.object({
  schemaVersion: z.number().min(0),
  engineVersion: z.string().min(1),
  scoringVersion: z.string().min(1),
  generatedAt: z.string().min(1),
  project: z.object({ name: z.string().min(1), directory: z.string().min(1) }).strict(),
  ...
  sessions: z.object({
    ...
    longSessions: z.number().min(0),
    failedSessions: z.number().min(0),
    repeatedFailures: z.number().min(0),
    compactions: z.number().min(0),
    permissionInterruptions: z.number().min(0),
  }).strict(),
