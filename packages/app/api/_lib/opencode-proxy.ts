...
const HOP_BY_HOP = new Set([
  "connection", "content-encoding", "content-length", "etag", "host", "keep-alive",
...
export async function proxyOpenCodeRequest(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  try {
    ...
    if (!target) return writeJson(res, 400, "Missing target server parameter") as never
    ...
    const response = await fetch(upstream, { method, headers: forwardedHeaders(req), body, duplex: body ? "half" : undefined } as RequestInit & { duplex?: "half" })
