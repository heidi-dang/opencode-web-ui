...
export default async function handler(req: RequestWithUrl, res: ServerResponse) {
  try {
    ...
    const route = incoming.pathname.replace(/^\/api\/opencode(?:\/api\/opencode)?/, "") || "/"
    ...
    for (const [key, value] of Object.entries(req.headers || {})) {
      if (!HOP_BY_HOP.has(key.toLowerCase()) && key.toLowerCase() !== "accept-encoding" && value !== undefined) headers.set(key, Array.isArray(value) ? value.join(", ") : value)
    }
